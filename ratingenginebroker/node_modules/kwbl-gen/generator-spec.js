...
describe(".random", function () {
  ...
  it("(min, max) :: [min > max] max - min", function () {
  	var min = 500;
  	var max = 200;
  	for (var i = 0; i < times; i++) {
  		var r = gen.random(min, max);
  		expect(r).toBeGreaterThan(max - 1);
  		expect(r).toBeLessThan(min + 1);
  	};
  });
...
describe(".duration", function () {
  it("()", function () {
  	for (var i = 0; i < times; i++) {
  		var r = gen.duration();
  		expect(r).toBeGreaterThan(0 - 1);
  		expect(r).toBeLessThan(30*60 + 1);
  	};
  });
});

describe(".cost", function () {
  it("() :: 0 - 100000", function () {
  	for (var i = 0; i < times; i++) {
  		var r = gen.cost();
  		expect(r).toBeGreaterThan(0 - 1);
...
describe(".couchTimeStamp", function () {
  it("()", function () {
  	var d = gen.couchTimeStamp();
  	checkDate(gen.couchTimeStamp());
  });

  it("(shift)", function () {
  	var shift = -24*60*60*1000;
  	var d1 = gen.couchTimeStamp();
  	var d2 = gen.couchTimeStamp(shift);
  	checkDate(d2);
  	expect(Math.floor(d1.t/1000) - Math.floor(d2.t/1000)).toEqual(-shift/1000);
  });


  it("(date)", function () {
  	var date = new Date('2010-09-10');
  	var d = gen.couchTimeStamp(date);
  	checkDate(d);
  	expect(d.Y).toEqual(2010);
  	expect(d.M).toEqual(9);
  	expect(d.D).toEqual(10);
  });

  it("(date, -shift)", function () {
  	var date = new Date('2010-09-10');
  	var shift = -24*60*60*1000;
  	var d = gen.couchTimeStamp(date, shift);
  	checkDate(d);
  	expect(d.Y).toEqual(2010);
  	expect(d.M).toEqual(9);
  	expect(d.D).toEqual(9);
  });
  ...
  it("(date, +shift)", function () {
  	...
  	expect(d.Y).toEqual(2010);
  	expect(d.M).toEqual(9);
  	expect(d.D).toEqual(11);
  });
});

describe(".scriptName", function () {
  it("(path) :: [__filename] 'generator-spec'", function () {
...
function checkDate(d){
	expect(typeof d === 'object').toBe(true);
  expect(typeof d.t === 'number').toBe(true);
  expect(typeof d.Y === 'number').toBe(true);
  expect(typeof d.M === 'number').toBe(true);
  expect(typeof d.D === 'number').toBe(true);
  expect(typeof d.h === 'number').toBe(true);
  expect(typeof d.m === 'number').toBe(true);
  expect(typeof d.s === 'number').toBe(true);
  expect(typeof d.z === 'string').toBe(true);
}